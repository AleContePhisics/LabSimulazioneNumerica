...
void System :: initialize_velocities(){
  ...
  if(_restart){
    ...
    for (int i=0; i<_npart; i++){     //codice per la delta delle velocità//
      if(i%6==0) vx(i)=sqrt(_temp);
      if(i%6==1) vy(i)=sqrt(_temp);
      if(i%6==2) vz(i)=sqrt(_temp);
      if(i%6==3) vx(i)=-sqrt(_temp);
      if(i%6==4) vy(i)=-sqrt(_temp);
      if(i%6==5) vz(i)=-sqrt(_temp);
      sumv(0)+=vx(i);
      sumv(1)+=vy(i);
      sumv(2)+=vz(i);
    }
...
void System :: measure(){ // Measure properties
  ...
  if(_measure_pofv){
    for(int j=0;j<_npart;j++){
      bin=floor(sqrt(dot(_particle(j).getvelocity(),_particle(j).getvelocity()))/_bin_size_v);
...
void System :: averages(int blk){
  ...
  if(_measure_pofv){
    ...
    for(int i=0;i<_n_bins_v;i++){
      ...
      coutf << setw(12) << _bin_size_v*i+_bin_size_v/2.  
            << setw(12) << sum_average/double(blk)
            << setw(12) << this->error(sum_average, sum_ave2, blk) << endl;
    }
    coutf.close();
