...
void System :: measure(){ // Measure properties
  ...
  double virial=0.0;
  int bin;
  vector<int>Gofr(_n_bins);
  //ofstream out;
  //out.open("Risultati.dat",ios::app);
  ...
  if (_measure_penergy or _measure_pressure or _measure_gofr) {
    for (int i=0; i<_npart-1; i++){
      for (int j=i+1; j<_npart; j++){
        ...
        if(_measure_gofr){
            bin=floor(dr/_bin_size);
            if(bin<_n_bins)  _measurement(bin+_index_gofr)+=2;
        }
...
void System :: averages(int blk){
  ...
  if (_measure_pressure){
    ...
    coutf << setw(13) << blk
          << setw(13) << average
          << setw(13) << sum_average/double(blk)
          << setw(13) << this->error(sum_average, sum_ave2, blk) << endl;
    coutf.close();
  }
  ...
  if (_measure_gofr){
    ...
    for (int i=0;i<_n_bins;i++) {
        ...
        double r1=r;
        double r2=r+_bin_size;
        double shell_volume=(4.0/3.0)*M_PI*(pow(r2,3)-pow(r1,3));
        double norm=_npart*_rho*shell_volume;
...
double System :: error(double a, double b, int blk){
    double err;
    if(blk==0){err=0;}
    else {
        err=sqrt((b/double(blk)-pow(a/double(blk),2))/double(blk));
    }
    return err;
}


void System :: AutoCorrelation(int _nsteps,int step,vector<double>&X){  
  ...
  double sum4=0;
  double sum5=0;
  double C=1./double(_nsteps-step);

  for(int i=0;i<_nsteps-step;i++){
      sum+=avgU[i]*avgU[i+step];
      sum2+=avgU[i];
      sum3+=avgU[i+step];
  }
  ...
  X[step]=(C*sum-(C*sum2)*(C*sum3))/((1./_nsteps)*sum4-pow((1./_nsteps)*sum5,2));
...
double System :: StatisticUncertainties(int L,int N){
    double sum=0,sum2=0,err;
    vector<double>avg(N);
    vector<double>avg2(N);
    for(int j=0;j<N;j++){
        for(int i=0;i<L;i++){
        sum+=avgU[i+L*j];
        }
    ...
    sum2=accumulate(avg2.begin(),avg2.end(),0.);
  
    err=error(sum,sum2,N);
