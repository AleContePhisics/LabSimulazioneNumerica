...
void System :: move(int i){ // Propose a MC move for particle i
  if(_sim_type == 3){ //Gibbs sampler for Ising
    ...
    r=_rnd.Rannyu();    //da aggiungere il campo H esterno
    p=1./(1+exp(-2*_beta*(_H+_J*(_particle(this->pbc(i-1)).getspin()+_particle(this->pbc(i+1)).getspin()))));
...
void System :: measure(){ // Measure properties
  ...
  // SPECIFIC HEAT /////////////////////////////////////////////////////////////
  if(_measure_cv)_measurement(_index_cv) = pow(_measurement(_index_tenergy),2);
  
  // SUSCEPTIBILITY ////////////////////////////////////////////////////////////
  if(_measure_chi){
    double sum=0;
    for(int i=0;i<_npart;i++){
      sum+=_particle(i).getspin();
    }
    sum=pow(sum,2);
    chi_temp=_beta*sum/_npart;
...
void System :: averages(int blk){
  ofstream kout;
  kout.open("../OUTPUT/Risultati.dat", ios::app);
  ofstream coutf;
  double average, sum_average, sum_ave2;
  vector<double> cv;
  cv.push_back(_measurement(_index_cv));
  ...
  if (_measure_tenergy){
    ...
    coutf << setw(12) << blk
          ...
          << setw(12) << this->error(sum_average, sum_ave2, blk) << endl;
    coutf.close();
    if(blk==get_nbl()) kout<<_temp<<"  ,  "<< sum_average/double(blk) <<"  ,  "<<this->error(sum_average, sum_ave2, blk)<<"   ,   ";
  }
  // TEMPERATURE ///////////////////////////////////////////////////////////////
  if (_measure_temp){
  ...
  // MAGNETIZATION /////////////////////////////////////////////////////////////
  if(_measure_magnet){
    coutf.open("../OUTPUT/magnet.dat",ios::app);
    ...
    coutf << setw(12) << blk
          ...
          << setw(12) << this->error(sum_average, sum_ave2, blk) << endl;
    coutf.close();
    if(blk==get_nbl()) kout<< sum_average/double(blk) <<"   ,  "<<this->error(sum_average, sum_ave2, blk)<<"   ,   ";
  ...
  if(_measure_cv){
    ...
    _global_av2(_index_cv)-=pow(_average(_index_cv),2);
  ...
  // SUSCEPTIBILITY ////////////////////////////////////////////////////////////
  if (_measure_chi){
    coutf.open("../OUTPUT/chi.dat",ios::app);
    ...
    coutf << setw(12) << blk
          ...
          << setw(12) << this->error(sum_average, sum_ave2, blk) << endl;
    coutf.close();
    if(blk==get_nbl()) kout<< sum_average/double(blk) <<"  ,   "<<this->error(sum_average, sum_ave2, blk)<<endl;
...
void System :: SetTemp(double T){
  _temp+=T;
  _beta=1./_temp;
}

void System :: SetSymType(int x){
  _sim_type=x;
}

void System :: SetH(double H){
  _H=H;
}
/****************************************************************
*****************************************************************
