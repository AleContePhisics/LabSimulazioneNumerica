...
void Population::Initialization() {
    int p1 = 0, p2 = 0;
    int rank;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);  // ottieni il rank

    // Leggi la riga del file Primes corrispondente al rank
    ifstream Primes("../INPUT/Primes");
    if (!Primes.is_open()) {
        cerr << "[Rank " << rank << "] Errore apertura file Primes!" << endl;
        MPI_Abort(MPI_COMM_WORLD, 1);
    }

    for (int i = 0; i <= rank; ++i) {
        if (!(Primes >> p1 >> p2)) {
            cerr << "[Rank " << rank << "] Errore lettura riga " << rank << " da Primes!" << endl;
            MPI_Abort(MPI_COMM_WORLD, 1);
        }
    }
    Primes.close();
    ...
    if (!Seed.is_open()) {
        cerr << "[Rank " << rank << "] Errore apertura file seed.in!" << endl;
        MPI_Abort(MPI_COMM_WORLD, 1);
    }
    ...
    for (int i = 0; i < n_genes; i++) {
        Popul[0].SetGene(i + 1, i);  // primo cromosoma ordinato
    }

    for (int i = 1; i < n_chromo; i++) {
        Popul[i] = Popul[0];
    }
...
void Population :: CitiesConfig(unsigned int k,int rank){ 
    double theta;
    ...
    if(k==0){    
        ...
        MPI_Barrier(MPI_COMM_WORLD); // sincronizza tutti i processi qui  
        MPI_Bcast(xcity.data(), n_genes, MPI_DOUBLE, 0, MPI_COMM_WORLD);
        MPI_Bcast(ycity.data(), n_genes, MPI_DOUBLE, 0, MPI_COMM_WORLD);
        for(int i=0;i<n_genes;i++){ 
            for(int j=0;j<n_genes;j++){
        ...
        ifstream in;
        in.open("../INPUT/cap_prov_ita.dat");
        for(int i=0;i<n_genes;i++){
            in>>xcity[i]>>ycity[i];
        }
        for(int i=0;i<n_genes;i++){
...
void Population::GetBestPath(unsigned int ex, int rank) {
    string filename;
    if (ex == 0) filename = "../OUTPUT/CirclePath" + to_string(rank) + ".dat";
    else if (ex == 1) filename = "../OUTPUT/SquarePath" + to_string(rank) + ".dat";
    else if (ex == 2) filename = "../OUTPUT/EuropePath" + to_string(rank) + ".dat";
    else if (ex == 3) filename = "../OUTPUT/ItalyPath" + to_string(rank) + ".dat";
    else {
        cerr << "Invalid ex parameter\n";
        exit(1);
    }
...
void ScambioBestChromosome(Population& Popul, int rank, int size, std::vector<int>& pairing, int M) {
    int partner = pairing[rank];

    std::vector<unsigned int> my_vec = Popul.GetChromo(0).ToVector();
    std::vector<unsigned int> recv_vec(my_vec.size());

    if (rank < partner) {
        MPI_Send(my_vec.data(), my_vec.size(), MPI_UNSIGNED, partner, 0, MPI_COMM_WORLD);
        MPI_Recv(recv_vec.data(), recv_vec.size(), MPI_UNSIGNED, partner, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    } else {
        MPI_Recv(recv_vec.data(), recv_vec.size(), MPI_UNSIGNED, partner, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        MPI_Send(my_vec.data(), my_vec.size(), MPI_UNSIGNED, partner, 0, MPI_COMM_WORLD);
    }
    ...
    Chromosome received;
    received.FromVector(recv_vec);
    Popul.SetChromo(received, M - 1);
}
